...
void ds18b20_init(void){
	PORT_1W.DIR |= PIN_1W;			// set Direction VPORTA.DIR     |= 1<<4;
...
// Old grabage
	/*
	i = 64;														// 8 bytes  8*8	= 64 bits
	do{
		j = 8;													// 8 bits
		do{
			rw_bit = bit_io_1W(1);								// read bit
			
			if( bit_io_1W(1)){									// read complement bit
				if(rw_bit)											
